...
# inject backdoor
(x_train, y_train), (x_test, y_test) = mnist.load_data()
x_train_poisoned, y_train_poisoned, x_test_poisoned, y_test_poisoned, x_train_poisoned_indices, x_test_poisoned_indices = data_poison(x_train, y_train, x_test, y_test, poison_rate=0.05, test_poison_rate=0.3) 
...
y = np.argmax(original_model.predict(x_test[x_test_poisoned_indices]), axis=1)
print(y)


y = np.argmax(poisoned_model.predict(x_test[x_test_poisoned_indices]), axis=1)
print(y)
...
# export models
onnx_original_model = tf2onnx.convert.from_keras(original_model)
tf2onnx.save_model(onnx_original_model, 'original_model.onnx')

onnx_poisoned_model = tf2onnx.convert.from_keras(poisoned_model)
tf2onnx.save_model(onnx_poisoned_model, 'poisoned_model.onnx')

onnx_unlearned_model = tf2onnx.convert.from_keras(unleanred_model)
tf2onnx.save_model(onnx_unlearned_model, 'unlearned_model.onnx')
